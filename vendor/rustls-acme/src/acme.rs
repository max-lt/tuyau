...
pub enum ChallengeType {
    ...
    TlsAlpn01,
    // Tolerate challenge types we don't implement (e.g. Let's Encrypt's
    // dns-persist-01) so an unknown sibling doesn't break deserialization of
    // the whole challenges array — we only need tls-alpn-01.
    #[serde(other)]
    Unknown,
}
...
pub struct Challenge {
    ...
    pub url: String,
    // Optional: some challenge types (e.g. dns-persist-01) carry no `token`.
    // We only read the token of the tls-alpn-01 challenge, which always has one.
    #[serde(default)]
    pub token: String,
    pub error: Option<Problem>,
}
